...
6. **Provider-health retry budget exhausted on a single call** — every chat call (worker and evaluator) is health-checked against the provider's own signals: an `error` object in the response, `finish_reason: "error"`, or a completely empty body (observed: OpenRouter 200s with zero usage during warm-up/scale-up windows). Unhealthy responses are retried with the message history untouched — they never become conversation turns and never consume iterations — with exponential backoff (2s doubling to a 60s cap, 8 attempts ≈ 3 minutes of patience, sized to how long real provider blips last). Exhaustion marks the task `failed` with reason `provider_failure` and halts the run, but the session status stays **`running`** — unlike caps 4, 5 and 7, this stop says nothing about the work, so it's classified like a token-cap stop: resumable. Unhealthy calls can cost no tokens, so the token cap can't catch a bad endpoint — this gate does. Fixed policy; no env knob.
7. **3 consecutive worker turns without a case inside a single task** — the worker went quiet but never called `submit_case`. After 3 nudges (each logged as a `nudge` event) the task is marked `failed` with reason `no_case`, run halts, `tilth resume` retries. Fixed at 3; no env knob.

Caps 2 and 3 are **session-level**. Caps 4–7 are **task-level**. There is no per-call cap and no per-task token cap. Caps 6 and 7 aren't env-tunable — they're fixed circuit-breakers against a misbehaving endpoint or a worker that never presents its case. That's the full safety story.
