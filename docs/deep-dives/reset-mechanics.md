...
1. **Resolve the session.** Bare `tilth reset` picks the latest by directory name (parallel to `tilth resume`); explicit `tilth reset <id>` targets that one.
...
5. Each step is **idempotent** — already-missing pieces are reported as skipped, not errored. You can run `tilth reset` against a half-cleaned-up state and it'll finish the job.
