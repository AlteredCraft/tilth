...
A Tilth session is append-only on disk: an `events.jsonl` log plus a `checkpoint.json` snapshot under `sessions/<id>/`. That's enough state to **resume** the run on a fresh process — or, when you're done with it, to **reset** (tear it down) in one command.
...
Continuing the [cap-hit example](../deep-dives/two-loops.md#what-can-stop-a-run) — same session (`20260523-082151-45f0a5`), with `TILTH_MAX_ITERATIONS_PER_TASK` bumped from `8` to `16` in `.env` before resuming (this example was captured when the cap was `8`, below today's default of `32`):
...
- **Preserves the token total.** If the original run hit `TILTH_MAX_TOKENS`, bump it in `.env` before resuming or the new run stops on the first token check.
...
A run lives on disk in two places (working tree under Tilth's `sessions/`; branch in the source repo's `.git` — see [Session layout](../deep-dives/session-layout.md)), and reset tears down both halves:

1. Reads `sessions/<id>/checkpoint.json` for the worktree path and branch; reads the `session_start` event for the source repo path.
