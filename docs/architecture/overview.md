...
7. **Session state belongs to the harness, not the source repo.** The working tree for a run lives at `sessions/<id>/workspace/` *inside Tilth* (gitignored); only the branch `session/<id>` and its worktree admin entry land in the source repo's `.git`. The source repo stays pristine — no `.gitignore` tax, no harness directories appearing in `ls`. See [Session layout](../deep-dives/session-layout.md).
