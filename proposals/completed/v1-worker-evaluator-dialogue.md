...
The escape-from-broken-task case (F7) is **not** resolved by v1 — the only termination path remains the existing iter/judge cap, which ends the entire run. This is unchanged from today and is a known limitation. The bet is that the ledger from v1 sessions will give us the empirical data needed to design a real halt mechanism (eligibility, terminal-state semantics, downstream-task implications) without guessing.
