...
**Reverted attempt:** [PR #17](https://github.com/AlteredCraft/tilth/pull/17) tried to address one face of F3/F6 by embedding the seed test in the evaluator prompt; reverted because it didn't actually move the loop's failure rate.

## Why this document

We've had enough seeded runs to see the same shape of failure recur across sessions: tasks burning 40+ iterations on what should be 8-iteration work, evaluator rejects the worker can't escape without gaming the rule, contradictions baked in at seed time that the run-time architecture can't recover from. Patching the surface keeps shipping work without moving the underlying behavior.
...
**Cost:** Every downstream actor (worker, validator, evaluator) treats the seed as ground truth. A seed contradiction becomes an unresolvable rejection loop that can only be detected by the human reviewing logs after the run has burned tokens.

### F2. Acceptance criteria and description can contradict

A specialization of F1, named separately because it's the most common shape. The `description` is what the worker reads and acts on. The `acceptance_criteria` are what the evaluator evaluates against. These can drift:

- Description authorizes a command whose side effects the AC doesn't authorize. (F1's example.)
- Description names a specific implementation path; AC names only the observable outcome. Worker takes the path; evaluator accepts. (No harm, but the convention isn't load-bearing.)
- Description is vague where AC is precise. Worker reads description, implements something that satisfies it loosely; evaluator measures against AC and rejects. (Common in human-written PRDs too — but the seeder is supposed to produce *better* than human-average here.)
...
The seeder writes tests. The worker satisfies them (and is told not to modify them). The evaluator evaluates the diff (and per the now-shipped `commit_seed`, doesn't see test files because they're in HEAD — see F6). Validators run them mechanically.
...
PR #17 (reverted) tried to address part of this — let the evaluator see the test file and explicitly reject it. The instinct was right; the surface-level fix didn't move outcomes in the trial run.
...
The evaluator's hard reject for scope creep (`tilth/prompts/evaluator.md`): if the diff modifies any file not in this task's AC, reject. The rule exists for a real reason — without it, workers pre-emptively implement future tasks (observed before the rule was added).

But from the diff alone, the evaluator can't distinguish:
...
The diff is identical in pairs 1, 2 and pairs 3, 4. The evaluator has no way to disambiguate.
...
5. Evaluator rejects on scope creep at iter 20.
...
### F6. The evaluator's information asymmetry

The evaluator sees, per `_evaluator_task`:
- Task description + AC
- AGENTS.md
...
- Prior evaluator calls' reasoning (each call is fresh-context).
...
It then has to make a binary accept/reject call with no memory and a narrow window. The evaluator has the smallest information surface of any actor in the system but the highest-stakes decision.
...
When a task fails (iter cap or evaluator cap), the whole run halts (`_run_session` returns on first failure). One bad seed → entire run lost.
...
`docs/deep-dives/agent-visibility.md` documents the rule: the worker doesn't see harness mechanics. The rule exists for real reasons (gaming the evaluator, token shortcuts, self-managing state — all observed failure modes before the wall).
...
- #16's session: cosmetic one-line edit to `tests/test_t001_scaffold.py` to get the file into the diff and clear a "file must exist" AC the evaluator couldn't see was already satisfied.
- `173822` session: added `[tool.ruff] exclude = [...]` to silence ruff noise from future-task tests it couldn't otherwise quiet.
- (Observed in earlier runs, not in current sessions:) padding commits with explanatory comments aimed at the evaluator.
...
**Cost:** False positives. The run finishes; the work compiles and passes tests; the seed contract has been technically met; but the diff includes cosmetic edits, exclude rules, and evaluator-aimed prose that nobody asked for and nobody benefits from.

## Cross-cutting observations

A few things that aren't single frictions but emerge across them:

- **The seeder is the highest-leverage actor in the system, but it gets the least feedback.** Every other actor (worker, evaluator, validator, self-improve) runs many times per session and gets evaluated against artifacts. The seeder runs once, terminal, and the only feedback is "did the run go well overall" — which arrives too late to influence the seed it wrote.
- **The architecture's "fresh context per evaluator call" + "agent doesn't see mechanics" + "no cross-task awareness" are individually defensible but jointly create blindness.** No actor has the view needed to catch architectural-level failures.
...
- **Tilth's quality gate is "tests pass + evaluator accepts," and we've seen both pass with broken work** (PR #17 was reverted partly because it didn't change this). The gate has shape but not teeth.
- **The harness was designed to keep the worker honest. It currently doesn't have machinery to keep the seeder honest, the evaluator honest, or itself honest about the seeder.**
